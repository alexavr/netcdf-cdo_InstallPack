...
#     Both fixed here
# eccodes issue: needs lib64 for compilers (so the latest compilers)
# proj issue: 
...
CDOPATHFLODER="/opt/netcdf/4.9.2/intel/2022.2.1/"
CDONAME="parallel"
# 2. Filenames to install 
# The elements are placed in the installation order, so it is RELEVANT!!! 
TARLIST=(
  # "szip-2.1.1.tar.gz"
  # "zlib-1.3.1.tar.gz"           # NETCDF && WRF & WPS GRIB2  & CDO
  # "hdf5-1.12.1.tar.gz"          # NETCDF && WRF & WPS GRIB2  & CDO
  # "netcdf-c-4.9.2.tar.gz"       # NETCDF && WRF & WPS GRIB2  & CDO
  "netcdf-fortran-4.6.1.tar.gz" # NETCDF && WRF & WPS GRIB2  & CDO
  # "jasper-1.900.1.tar.gz"       # WPS GRIB2  & CDO
  # "libpng-1.2.50.tar.gz"        # WPS GRIB2  & CDO
...
export CC=mpiicc # gcc #/usr/local/bin/gcc
# export CXX=icpc # g++ #/usr/local/bin/g++
# export CPP="mpiicc -E"
# export CXXCPP='mpiicpc -E'
export FC=mpiifort #/usr/local/bin/gfortran
export F77=mpiifort # /usr/local/bin/gfortran
...
for i in ${TARLIST[@]}; do 
  ...
  if [ "$SOFTNAME" == "zlib" ]; then
    ...
    execute "./configure --prefix=$CDOPATH  \
                     --with-szlib=$CDOPATH/lib  \
                     --with-zlib=$CDOPATH/lib   \
                     --enable-parallel      \
                     --enable-fortran    \
                     &> _configure.log"
...
echo "Everythig was compiled for static libs."
