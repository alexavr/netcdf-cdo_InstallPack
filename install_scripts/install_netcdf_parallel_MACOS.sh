...
TMP=${TARLIST[-1]}
CDONAME=${TMP%.tar.gz*}
export CDOPATH=$CDOPATHFLODER/netcdf4-parallel
export CC=mpicc
export CXX=mpicxx
export CPP="mpicc -E"
...
export FC=mpifort
export F77=mpifort
#export CFLAGS="-O2 -xHost -ip -fPIC"   # "-O3 -xHost -ip -no-prec-div -fPIC"    # "-O3 -xHost -ip -fPIC"  -static-intel 
...
for i in ${TARLIST[@]}; do 
  ...
  if [ "$SOFTNAME" == "zlib" ]; then
    ...
    execute "./configure --prefix=$CDOPATH  \
                     --with-szlib=$CDOPATH  \
                     --with-zlib=$CDOPATH   \
                     --with-pic --with-pthread --disable-sharedlib-rpath --disable-cxx  \
                     --enable-parallel      \
