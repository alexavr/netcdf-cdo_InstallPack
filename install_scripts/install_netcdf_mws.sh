...
export FC=ifort
export F77=ifort
export CFLAGS="-fPIC"   # "-O3 -xHost -ip -no-prec-div -fPIC"    # "-O3 -xHost -ip -fPIC"  -static-intel 
export FCFLAGS="-fPIC" # "-O3 -xHost -ip -fPIC"  #"-O3 -xHost -ip -no-prec-div -fPIC"    # "-O3 -xHost -ip -fPIC"  -static-intel 
...
for i in ${TARLIST[@]}; do 
  ...
  if [ "$SOFTNAME" == "zlib" ]; then
    ...
    execute "make -j 4 &> _make.log "
    execute "make -j 4 check &> _make_check.log"
    execute "make install &> _make_install.log"
